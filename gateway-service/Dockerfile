...
ENTRYPOINT java -javaagent:/app/gateway-service/elastic-apm-agent.jar \
            -Delastic.apm.server_urls=$ELASTIC_APM_SERVER_URLS \
            -Delastic.apm.service_name=gateway-service \
            -Delastic.apm.environment=develop \
            -Delastic.apm.secret_token=$ELASTIC_APM_SECRET_TOKEN \
            -Delastic.apm.application_packages=com.alphaka.gatewayservice \
